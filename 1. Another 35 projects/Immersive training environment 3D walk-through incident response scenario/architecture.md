...
also be packaged as a desktop EXE (built: `release\IRT_Incident_Simulator-
win32-x64\IRT_Incident_Simulator.exe`; shell source under `shell/`).
...
├── package.json          # root metadata (name/main for Electron packager)
├── shell/
│   ├── main.js           # Electron main process (loads index.html)
│   └── package.json      # electron + electron-packager dev deps
├── assets/
